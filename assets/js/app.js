...
window.doLogin=()=>{
  ...
  const isTech = found.role==='tech';
  document.getElementById('nav-logs').style.display = isTech ? 'none' : 'flex';
  document.getElementById('nav-alerts').style.display = isTech ? 'none' : 'flex';
...
window.showPage=(page)=>{
  if(CU && CU.role==='tech' && ['logs','alerts'].includes(page)){
    toast('Acesso restrito para técnicos.','🔒','t-red');
    return;
  }
